...
var IdSecObfuscate = function(password, cipherAlgorithm, blockSize) {
...
IdSecObfuscate.prototype.updateAvailableCiphers = function(ciphers, availableCiphers) {
  var i=0;
...
IdSecObfuscate.prototype.createNullArray = function(length) {
  return Array.apply(null, new Array(length)).map(Number.prototype.valueOf,0);
}

IdSecObfuscate.prototype.encrypt = function(number) {
  var cipher = crypto.createCipher(this.cipherAlgorithm, this.password),
    bufferArray = [],
  ...
  buffer = new Buffer(bufferArray);
  encryptedNumber = cipher.update(buffer, 'binary', 'hex') + cipher.final('hex');
  
  return encryptedNumber;
}

IdSecObfuscate.prototype.decrypt = function(encryptedNumber) {
  var decipher = crypto.createDecipher(this.cipherAlgorithm, this.password),
